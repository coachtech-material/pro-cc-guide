...
[Chapter 3-4「機能を開発する」](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md) で機能を開発したとき、`CoachCourseController@store` が 200 行近い Fat Controller であることに触れました。また、コース一覧や生徒一覧の画面では、データベースへの問い合わせが非効率に行われている可能性があります。

どちらも「動いている」コードです。機能は正しく動作しています。しかし、このままでは次にこのコードを変更するとき（機能追加やバグ修正のとき）に余計な時間がかかります。Fat Controller は200 行の中から該当箇所を探す必要があり、N+1 クエリはデータが増えるほど画面の表示が遅くなります。

この Chapter では、3-5-1 で学んだリファクタリングのフロー（対象特定→テスト確認→改善→テスト再実行）を CourseHub で実践します。

### 🧠 先輩エンジニアはこう考える

> リファクタリングのきっかけとして一番多いのは、機能開発やバグ修正で関連コードを読んでいるときに問題点に気づくパターンだ。ただし、3-3-1 で学んだ通り、バグ修正やリファクタリングを同じコミットに混ぜるのは NG。気づいた問題はメモしておいて、タスクを分けて着手する。
>
> 今回は [Chapter 3-4「機能を開発する」](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md) の機能開発が終わったタイミングでリファクタリングに取り組む。これも実務では自然な流れ。機能を追加する過程でコードの問題点が見えてくるから、それを別のタスクとして整理する。

---

## 🏃 実践①: Fat Controller の責務分離（コード構造の改善）

> 📌 前の Chapter のブランチにいる場合は、main ブランチに戻してから作業を開始してください。各 Chapter の変更は独立したブランチで管理し、Chapter の完了時にコミットしておくことで、後から PR にまとめやすくなります。
...
合計で約 200 行近いメソッドです。これは明確な Fat Controller（3-5-1 で学んだコードスメル）です。
...
> 💡 3-1-3 で規約と実態のズレを確認しましたね。`CoachCourseController` はそのズレの1つでした。ここでそれを実際に修正します。
...
3-5-1 で学んだ通り、**テストがない機能のリファクタリングは危険** です。リファクタリング前に、カバーされていない処理のテストを追加しましょう。
...
>
> 「動くコードが書ける」ことと「このプロジェクトに適切な設計判断ができる」ことは別です。Claude Code の提案を鵜呑みにせず、プロジェクトの現状に合った判断をすることが、3-5-1 で学んだ「改善提案を評価する3つの基準」の実践です。
...
次に、パフォーマンスの問題を改善します。CourseHub の2つの一覧画面で N+1 クエリが発生しています。
...
CourseHub の2つの一覧画面で N+1 クエリが発生しています。Claude Code に分析を依頼しましょう。

```
> CourseController@index（コース一覧）と AdminStudentController@index（生徒一覧）で
> N+1 クエリが発生していないか分析してください。
...
**生徒一覧（`AdminStudentController@index`）**
...
Blade テンプレートでアクセスしているリレーション:
- `$student->enrollments->count()` → 受講コース数（N+1）

20 件の生徒がある場合、1 + 20 = **21 回のクエリ** が実行されます。

</details>

### Worktree で並列に改善する

3-5-1 で学んだ通り、パフォーマンス改善は **異なるファイルの問題を並列に作業できる** 特徴があります。コース一覧の N+1 と生徒一覧の N+1 は、関連するファイル（Controller、Blade テンプレート）が異なるため、同時に進められます。

ここで Claude Code の **Worktree** 機能を使います。Worktree は Git の仕組みを使って、リポジトリの別のコピーを作成し、そこで独立した作業を行う機能です。メインの作業ディレクトリに影響を与えずに、別のブランチで作業を進められます。

#### Worktree A: コース一覧の N+1 を解消する

ターミナルで新しい Worktree を起動します。

```bash
claude --worktree fix-n1-courses
```

新しい Claude Code セッションが Worktree 内で起動します。
...
> 💡 Worktree で作業が完了すると、Claude Code がブランチの扱い（マージまたは削除）を確認します。ひとまずブランチを残しておき、後でメインブランチにマージしましょう。

#### Worktree B: 生徒一覧の N+1 を解消する

別のターミナルで、もう1つの Worktree を起動します。Worktree A と **並列に** 作業できます。

```bash
claude --worktree fix-n1-students
```

```
> AdminStudentController@index の N+1 クエリを解消してください。
>
> 現状: User::where('role', 'student')->paginate(20) で
> Eager Loading なしのため、Blade で enrollments にアクセスするたびにクエリが発生しています。
>
> 修正方針:
> - withCount('enrollments') で受講コース数を取得する
> - Blade テンプレートで enrollments->count() を enrollments_count に変更する
>
> テストがあれば実行して動作確認してください。
```
...
- `withCount('enrollments')` を追加するだけで、20 回のクエリが 1 回のサブクエリに置き換わります
- 変更は Controller の1行追加と Blade テンプレートの1箇所修正のみ。最小限の変更でパフォーマンスが改善されます
- 21 回のクエリが 1〜2 回に削減されます
...
2つの Worktree での作業が完了したら、メインの作業ディレクトリに戻り、両方のブランチをマージします。

> 📝 `claude --worktree <name>` で作成されるブランチの名前は `worktree-<name>` の形式になります。ブランチ名が不明な場合は `git branch` で一覧を確認してください。

```bash
# メインの作業ディレクトリで現在のブランチにいることを確認
git branch

# Worktree のブランチをマージ
git merge worktree-fix-n1-courses
git merge worktree-fix-n1-students
```

マージ後にテストを実行して、統合に問題がないことを確認します。

```
/test
```

> 💡 **Worktree の威力**: コース一覧と生徒一覧の N+1 解消は、互いに関連するファイルがないため並列に作業できました。実務でも、独立した複数の修正を並行して進めたい場面は多くあります。Worktree はそうした場面で特に効果を発揮します。
...
- [ ] **正しさ**: コース一覧と生徒一覧の表示内容がリファクタリング前と同一か。テストが全て通っているか。`withCount()` で取得した件数が `count()` で取得した件数と一致しているか
...
|---|---|---|
| 改善対象 | 1ファイル（CoachCourseController） | 2画面（コース一覧 + 生徒一覧） |
| 事前準備 | テスト追加 + Plan Mode で計画 | 問題の特定（クエリ分析） |
| 作業の進め方 | 段階的（1責務ずつ抽出） | 並列（Worktree で同時作業） |
...
- [ ] N+1: 生徒一覧で `withCount('enrollments')` が追加されている
...
- **Worktree** を使えば、独立した改善を並列に進められる。関連ファイルが重ならない修正は同時作業で効率化する
