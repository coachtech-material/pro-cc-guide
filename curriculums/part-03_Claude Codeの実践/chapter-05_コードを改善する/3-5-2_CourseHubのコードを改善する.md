...
[3-5-1 リファクタリングの方法論](3-5-1_リファクタリングの方法論.md) で学んだ通り、パフォーマンス改善は **異なるファイルの問題を並列に作業できる** 特徴があります。コース一覧の N+1 と生徒一覧の N+1 は、関連するファイル（Controller、Blade テンプレート）が異なるため、同時に進められます。
