...
Chapter 3-4 で機能を開発したとき、`CoachCourseController@store` が 200 行近い Fat Controller であることに触れました。また、コース一覧画面では、データベースへの問い合わせが非効率に行われている可能性があります。
...
### Step 2: Eager Loading で N+1 を解消する

分析結果をもとに、Claude Code に修正を依頼します。
...
テストを実行して、表示内容が変わっていないことを確認します。

```
/test
```

### Step 4: コンテキスト使用量を確認する
...
> 💡 同様の N+1 問題は他の一覧画面（生徒一覧など）にも存在する可能性があります。同じパターン（`with()` / `withCount()` の追加 + Blade テンプレートの修正）で対応できるので、余裕があれば探して修正してみてください。
...
|---|---|---|
| 改善対象 | 1ファイル（CoachCourseController） | 1画面（コース一覧） |
| 事前準備 | テスト追加 + Plan Mode で計画 | 問題の特定（クエリ分析） |
| 作業の進め方 | 段階的（1責務ずつ抽出） | 問題分析 → 修正 → テスト |
