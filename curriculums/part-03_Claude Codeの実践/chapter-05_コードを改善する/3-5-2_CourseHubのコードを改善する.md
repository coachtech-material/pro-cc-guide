...
Chapter 3-4「機能を開発する」で機能を開発したとき、`CoachCourseController@store` が 200 行近い Fat Controller であることに触れました。また、コース一覧や生徒一覧の画面では、データベースへの問い合わせが非効率に行われている可能性があります。
...
>
> 今回は Chapter 3-4「機能を開発する」の機能開発が終わったタイミングでリファクタリングに取り組む。これも実務では自然な流れ。機能を追加する過程でコードの問題点が見えてくるから、それを別のタスクとして整理する。
