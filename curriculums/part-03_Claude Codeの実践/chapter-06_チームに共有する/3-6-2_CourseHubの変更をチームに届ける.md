...
- **[Chapter 3-1「実践の準備」](../chapter-01_実践の準備/3-1-0_Chapter%201の地図.md)**: CLAUDE.md の更新、rules の整備、skills の修正、settings.json の更新
- **[Chapter 3-3「バグを修正する」](../chapter-03_バグを修正する/3-3-0_Chapter%203の地図.md)**: 進捗率バグ、認可バグ、500 エラーの修正
- **[Chapter 3-4「機能を開発する」](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md)**: 小テスト再受験機能、コースレビュー機能の追加
- **[Chapter 3-5「コードを改善する」](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md)**: Fat Controller の責務分離、N+1 クエリの解消
...
以下は、[Chapter 3-5「コードを改善する」](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md) のリファクタリング（Fat Controller の責務分離）を PR にした場合の例です。
...
|---|---|---|
| コードレビュー（自己レビュー） | [3-6 チームに共有する](3-6-0_Chapter%206の地図.md)（実践①） | 高: PR 前に毎回実行する |
| バグ調査（症状→再現→調査→修正） | [3-3 バグを修正する](../chapter-03_バグを修正する/3-3-0_Chapter%203の地図.md) | 中: バグ報告のたびに使う |
| テスト実行 | [3-3](../chapter-03_バグを修正する/3-3-0_Chapter%203の地図.md), [3-4](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md), [3-5](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md) | 既存（`/test`）: [3-1-3 プロジェクトの規約・設定を確認する](../chapter-01_実践の準備/3-1-3_プロジェクトの規約・設定を確認する.md) で修正済み |
| リファクタリング計画 | [3-5 コードを改善する](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md) | 中: 改善タスクのたびに使う |
...
|---|---|---|---|---|
| [3-1 実践の準備](../chapter-01_実践の準備/3-1-0_Chapter%201の地図.md) | 環境と基盤 | 70-30 モデル、説明責任の導入 | セットアップ、規約・設定の確認 | - |
| [3-2 既存コードを理解する](../chapter-02_既存コードを理解する/3-2-0_Chapter%202の地図.md) | コードリーディング | 全体像→構造→フロー、知らない技術への向き合い方 | Sub-agents での探索、業務フロー追跡 | - |
| [3-3 バグを修正する](../chapter-03_バグを修正する/3-3-0_Chapter%203の地図.md) | バグ修正 | 報告→再現→調査→修正→検証 | 進捗率バグ、認可バグ、500 エラー | **安全性** |
| [3-4 機能を開発する](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md) | 機能開発 | 要件理解→設計→実装→検証 | 小テスト再受験、コースレビュー | **品質** |
| [3-5 コードを改善する](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md) | リファクタリング | 対象特定→テスト確認→改善→テスト再実行 | Fat Controller 分離、N+1 解消 | **正しさ** |
| [3-6 チームに共有する](3-6-0_Chapter%206の地図.md) | チーム開発 | 説明責任、レビュー、帰属表示 | PR 作成、GitHub Actions、Skill 化 | **品質** |
...
| Sub-agents | コードベースの広範な探索、複数ファイルの並列分析（[3-2 既存コードを理解する](../chapter-02_既存コードを理解する/3-2-0_Chapter%202の地図.md)） |
| MCP | データベースの直接確認、外部ツール連携（[3-2 既存コードを理解する](../chapter-02_既存コードを理解する/3-2-0_Chapter%202の地図.md)） |
| `/compact` / `/context` | 長い作業セッションでのコンテキスト管理（[3-2](../chapter-02_既存コードを理解する/3-2-0_Chapter%202の地図.md), [3-5](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md)） |
| `/btw` | コードリーディング中の補足質問（[3-2 既存コードを理解する](../chapter-02_既存コードを理解する/3-2-0_Chapter%202の地図.md)） |
| プロンプト設計 | バグの症状・再現手順・期待値を構造化して伝える（[3-3 バグを修正する](../chapter-03_バグを修正する/3-3-0_Chapter%203の地図.md)） |
| `/rewind` | 修正が期待通りでない場合の巻き戻し（[3-3 バグを修正する](../chapter-03_バグを修正する/3-3-0_Chapter%203の地図.md)） |
| `/test` | 修正・開発・リファクタリングの各フェーズでの動作確認（[3-3](../chapter-03_バグを修正する/3-3-0_Chapter%203の地図.md), [3-4](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md), [3-5](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md)） |
| セッション管理 | `--continue` / `--resume` で長い機能開発を途中再開（[3-4 機能を開発する](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md)） |
| Plan Mode | 設計の計画立案、段階的なリファクタリング計画（[3-4](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md), [3-5](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md)） |
| Hooks | コミット前の自動フォーマット（[3-4 機能を開発する](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md)） |
| `/simplify` | 機能開発後の品質チェック（[3-4 機能を開発する](../chapter-04_機能を開発する/3-4-0_Chapter%204の地図.md)） |
| Worktree | 独立した改善の並列作業（[3-5 コードを改善する](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md)） |
| `/cost` | トークン使用量の確認（[3-5 コードを改善する](../chapter-05_コードを改善する/3-5-0_Chapter%205の地図.md)） |
| `/commit` / `/diff` | 変更の確認とコミット（[3-6 チームに共有する](3-6-0_Chapter%206の地図.md)） |
| GitHub Actions | PR への自動レビュー（[3-6 チームに共有する](3-6-0_Chapter%206の地図.md)） |
| 権限管理 | settings.json のチーム共有設定（[3-6 チームに共有する](3-6-0_Chapter%206の地図.md)） |
| Skills | ワークフローの定義とチーム共有（[3-6 チームに共有する](3-6-0_Chapter%206の地図.md)） |
| Plugins | Skill のパッケージ化と配布（[3-6 チームに共有する](3-6-0_Chapter%206の地図.md)） |
