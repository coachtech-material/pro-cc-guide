...
- **Chapter 3-1「実践の準備」**: CLAUDE.md の更新、rules の整備、skills の修正、settings.json の更新
- **Chapter 3-3「バグを修正する」**: 進捗率バグ、認可バグ、500 エラーの修正
- **Chapter 3-4「機能を開発する」**: 小テスト再受験機能、コースレビュー機能の追加
- **Chapter 3-5「コードを改善する」**: Fat Controller の責務分離、N+1 クエリの解消
...
以下は、Chapter 3-5「コードを改善する」のリファクタリング（Fat Controller の責務分離）を PR にした場合の例です。
...
|---|---|---|
| コードレビュー（自己レビュー） | 3-6 チームに共有する（実践①） | 高: PR 前に毎回実行する |
| バグ調査（症状→再現→調査→修正） | 3-3 バグを修正する | 中: バグ報告のたびに使う |
| テスト実行 | 3-3 バグを修正する, 3-4 機能を開発する, 3-5 コードを改善する | 既存（`/test`）: [3-1-3 プロジェクトの規約・設定を確認する](../chapter-01_実践の準備/3-1-3_プロジェクトの規約・設定を確認する.md) で修正済み |
| リファクタリング計画 | 3-5 コードを改善する | 中: 改善タスクのたびに使う |
