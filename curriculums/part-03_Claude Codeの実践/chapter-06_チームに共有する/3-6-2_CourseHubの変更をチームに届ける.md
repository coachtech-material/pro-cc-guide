...
>
> 複数の Chapter で異なるブランチを作成した場合は、PR を作成する前に1つのブランチに統合する必要があります。Claude Code に「Chapter 3-3 から 3-5 のブランチを1つにまとめたい」と相談すれば、マージの手順を案内してくれます。または、以下のように手動でまとめることもできます。
>
> ```bash
> git checkout main
...
Part 3 で取り組んだ変更を PR にまとめます。ここでは、3-6-1 で学んだ説明責任を意識して PR を作成します。
...
3-6-1 で学んだ通り、PR を出す前に自己レビューを行います。Claude Code に依頼しましょう。
...
PR を作成したら、3-6-1 で学んだ「説明できるか」テストを行いましょう。PR の説明を読み返し、以下の質問に答えられるか確認します。
...
> 📌 GitHub Actions の利用には Anthropic API キーが必要です。2-3-8 で API キーを取得済みであることを前提とします。まだの場合は 2-3-8 の手順を参照してください。
...
- `--max-turns 5` でターン数を制限しています。API のコストを管理するための設定です。2-3-8 で学んだ通り、学習段階では 5 ターンで十分です
...
3-1-3 で `.claude/settings.json` と `.claude/settings.local.json` を更新しましたが、改めてチーム共有の観点で整理します。
...
> 📝 あなたの環境では、3-1-3 での更新内容に応じて異なる設定になっている場合があります。上記はチーム共有に最低限必要な設定の目安です。
...
3-1-3 で CLAUDE.md を更新しましたが、今後の運用方針を確認しておきましょう。

CLAUDE.md はプロジェクトの進行とともに更新される「生きたドキュメント」です。3-6-1 で学んだ通り、以下のタイミングで更新します。
...
| テスト実行 | 3-3, 3-4, 3-5 | 既存（`/test`）: 3-1-3 で修正済み |
...
| Sub-agents | コードベースの広範な探索、複数ファイルの並列分析（3-2 既存コードを理解する） |
| MCP | データベースの直接確認、外部ツール連携（3-2 既存コードを理解する） |
| `/compact` / `/context` | 長い作業セッションでのコンテキスト管理（3-2 既存コードを理解する, 3-5 コードを改善する） |
| `/btw` | コードリーディング中の補足質問（3-2 既存コードを理解する） |
| プロンプト設計 | バグの症状・再現手順・期待値を構造化して伝える（3-3 バグを修正する） |
| `/rewind` | 修正が期待通りでない場合の巻き戻し（3-3 バグを修正する） |
| `/test` | 修正・開発・リファクタリングの各フェーズでの動作確認（3-3 バグを修正する, 3-4 機能を開発する, 3-5 コードを改善する） |
| セッション管理 | `--continue` / `--resume` で長い機能開発を途中再開（3-4 機能を開発する） |
| Plan Mode | 設計の計画立案、段階的なリファクタリング計画（3-4 機能を開発する, 3-5 コードを改善する） |
| Hooks | コミット前の自動フォーマット（3-4 機能を開発する） |
| `/simplify` | 機能開発後の品質チェック（3-4 機能を開発する） |
| Worktree | 独立した改善の並列作業（3-5 コードを改善する） |
| `/cost` | トークン使用量の確認（3-5 コードを改善する） |
| `/commit` / `/diff` | 変更の確認とコミット（3-6 チームに共有する） |
| GitHub Actions | PR への自動レビュー（3-6 チームに共有する） |
| 権限管理 | settings.json のチーム共有設定（3-6 チームに共有する） |
| Skills | ワークフローの定義とチーム共有（3-6 チームに共有する） |
| Plugins | Skill のパッケージ化と配布（3-6 チームに共有する） |
