...
> 💡 Hooks は `.claude/settings.json` に保存されるため、チームで共有できます。プロジェクトの `.claude/settings.json` に Hooks を追加しておけば、チーム全員が同じ自動フォーマットの恩恵を受けられます。これについては [Chapter 3-6「チームに共有する」](../chapter-06_チームに共有する/3-6-0_Chapter%206の地図.md) で詳しく扱います。
