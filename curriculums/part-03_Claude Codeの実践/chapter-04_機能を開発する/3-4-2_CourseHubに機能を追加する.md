...
> 📌 CourseHub のプロジェクトディレクトリで Claude Code を起動していることを確認してください。

> 📌 [3-1-2 セットアップと動作確認](../chapter-01_実践の準備/3-1-2_セットアップと動作確認.md) で説明したとおり、各 Chapter のハンズオンは **必ず `main` ブランチから新しい作業ブランチを切って** 進めてください。前 Chapter のバグ修正ブランチに残ったままだと、Claude Code が修正後のコードを前提に動いてしまいます。`/clear` で会話履歴を消してもファイルの状態は残るため、Git でブランチを切り替えて素の状態に戻すことが必要です。

### Step 1: 作業ブランチを作成し、セッションに名前を付ける

まず、`main` ブランチから作業ブランチを切ります。

```
> main ブランチに切り替えて最新化したあと、feature/quiz-retake ブランチを作成してください
```

Claude Code が `git checkout main && git pull && git checkout -b feature/quiz-retake` を実行します。
...
[3-4-1 機能開発の方法論](3-4-1_機能開発の方法論.md) で学んだように、拡張のアプローチでは **まず既存コードを読む** ことから始めます。
...
> 📌 実践①のブランチ（`feature/quiz-retake`）にいる場合は、`main` に戻ってから新しい作業ブランチを切ります。`/clear` してもファイル状態は残るため、Git でブランチを切り替えて素の状態に戻してください。

### Step 0: 作業ブランチを作成する

`main` ブランチから新しい作業ブランチを切ります。

```
> main ブランチに切り替えて最新化したあと、feature/course-review ブランチを作成してください
```

### Step 1: 類似機能のコードを読む

新規機能の構築では、[3-4-1 機能開発の方法論](3-4-1_機能開発の方法論.md) で学んだように **類似機能のパターンを把握する** ことから始めます。コースレビューは「受講生がコースに対して操作する」機能なので、同じパターンの「受講登録（Enrollment）」の実装を参考にします。
...
Plan Mode を承認したら、Shift+Tab で Normal Mode に切り替え、段階的に実装します。[3-4-1 機能開発の方法論](3-4-1_機能開発の方法論.md) で学んだように、一度に全てを作らず、ステップごとに確認しながら進めます。
