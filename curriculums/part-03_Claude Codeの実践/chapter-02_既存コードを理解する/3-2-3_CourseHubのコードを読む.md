...
最初のステップは、CourseHub がどんなプロジェクトなのかを俯瞰的に把握することです。3-1-2 でセットアップし、3-1-3 で README やコーディング規約を確認しましたが、まだ「コードそのもの」には深く踏み込んでいません。
...
全体像を把握できたら、次は解像度を一段上げて、コードの構造を理解します。3-2-1 の Step 2 で学んだ「モデル・リレーション」「認可の仕組み」「設計パターン」に注目します。
...
コードから読み取ったモデルのリレーションが、実際のデータベースと一致しているかを確認します。3-2-1 で学んだ「Claude の説明を検証する」の実践として、MCP でデータベースに直接アクセスします。
...
これら5つの処理は、複数のモデル（`Enrollment`、`Course`、`Chapter`、`Lesson`、`LessonProgress`）にまたがっています。CourseHub の README の設計方針にも「複数モデルにまたがる処理は Service クラスに集約する」と書かれていました（3-1-3 で確認済み）。
...
> 📝 このように「規約や方針はあるが、一貫して適用されていない」状態は、実務のプロジェクトでは非常によくあります。前任チームの誰かが EnrollmentService を切り出し、他の Controller はそのまま残ったのでしょう。3-1-3 で確認した「規約と実コードの乖離」の具体例です。
