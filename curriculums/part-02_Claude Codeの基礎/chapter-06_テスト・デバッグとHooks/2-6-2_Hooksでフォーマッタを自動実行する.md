...
### PostToolUse で php-cs-fixer を自動実行する

仕組みがわかったところで、実際に設定しましょう。PHP のコードスタイルを統一する **PHP CS Fixer** を、Claude Code がファイルを編集するたびに自動で実行する Hook を設定します。

まず、PHP CS Fixer をプロジェクトにインストールします。

```
> PHP CS Fixer を開発用の依存パッケージとしてインストールしてください。sail composer require --dev friendsofphp/php-cs-fixer を使ってください。
```

インストールが完了したら、Hook を設定します。
...
> - タイプ: command
> - コマンド: PHP ファイルが編集されたときだけ php-cs-fixer を実行する。sail exec laravel.test を使って Sail コンテナ内で実行する
```
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "FILE=$(cat | jq -r '.tool_input.file_path') && if [[ \"$FILE\" == *.php ]]; then sail exec laravel.test ./vendor/bin/php-cs-fixer fix \"$FILE\" --quiet 2>/dev/null; fi"
          }
        ]
      }
    ]
...
2. **`cat | jq -r '.tool_input.file_path'`**: Hook はイベントデータを標準入力（stdin）で JSON 形式で受け取ります。`jq` で編集されたファイルのパスを取り出しています
3. **`if [[ "$FILE" == *.php ]]`**: PHP ファイルの場合のみフォーマッタを実行します。JavaScript や Markdown のファイルを編集したときは何も起きません
4. **`sail exec laravel.test`**: php-cs-fixer は PHP のツールなので、Sail コンテナ内で実行する必要があります
5. **`--quiet 2>/dev/null`**: フォーマッタの出力を抑制して、Claude Code のターミナルを見やすく保ちます
...
Claude Code がファイルを編集した後、自動的に php-cs-fixer が実行されます。ターミナルの verbose モード（`Ctrl+O`）を有効にすると、Hook の実行ログを確認できます。
...
> ⚠️ よくあるエラー: php-cs-fixer が見つからない
>
> ```
> sh: ./vendor/bin/php-cs-fixer: No such file or directory
> ```
>
> **原因**: php-cs-fixer のインストールが完了していないか、Sail コンテナ外で実行しようとしています。
>
> **対処法**: `sail composer show friendsofphp/php-cs-fixer` でインストール状況を確認してください。インストールされていれば、コマンドが `sail exec laravel.test` 経由で実行されているか確認してください。
...
- PostToolUse イベントと `Edit|Write` matcher を組み合わせて、PHP ファイル編集後に php-cs-fixer を自動実行する Hook を設定しました
