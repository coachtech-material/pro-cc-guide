...
>
> 1. User（Laravel標準のusersテーブルにroleカラムを追加）
>    - role: enum('admin', 'author'), デフォルト'author'
>
> 2. Category
>    - name: string(50)
...
**マイグレーション**:
- `role` カラムは `enum('admin', 'author')` で、デフォルトは `'author'` になっているか
...
- [ ] **正しさ**: マイグレーションのカラム型・制約が指示通りか（`role` は enum、`category_id` は nullable、`body_markdown` は text、`published_at` は nullable timestamp）
