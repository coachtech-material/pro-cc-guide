...
```
GitHub 上のイベント                  claude-code-action
┌────────────────────┐            ┌────────────────────┐
│ PR が作成された     │ ──────▶   │ コードを分析       │
│ Issue が作成された  │            │ レビューコメント   │
│ @claude とメンション │           │ コードの修正       │
│ された              │            │ PR の作成          │
└────────────────────┘            └────────────────────┘
                                          │
                                          ▼
...
| **料金体系** | 月額固定（Pro: $20/月、Max: $100〜$200/月） | 従量課金（トークン数に応じた料金） |
| **認証** | ブラウザでのログイン | API キー |
| **アカウント** | Claude.ai | Anthropic Console |

> ⚠️ **費用に関する重要な注意**: GitHub Actions で claude-code-action を使うと、ワークフローが実行されるたびに Anthropic API の利用料金が発生します。料金はタスクの複雑さ（消費トークン数）によって変わります。学習目的の小規模な利用であれば大きな金額にはなりませんが、意図しない大量実行を防ぐため、`max-turns` の設定やワークフローのタイムアウトを適切に設定してください。
...
```
> 作成したワークフローファイルをコミットして push して。コミットメッセージは「Add Claude Code GitHub Actions workflow」で
```

### Step 5: PR を作成して動作を確認する

ワークフローの動作を確認するため、簡単な変更を加えた PR を作成します。

```
> 新しいブランチ test/claude-review を作成して、README.md に「## テスト」セクションを追加して、PR を作成して
```
...
これで Chapter 2-3 の概念+実践ブロック（2-3-1〜2-3-8）は完了です。Plan Mode、Skills、Hooks、MCP、Sub-agents、Plugins、Git 連携、GitHub Actions という Claude Code の主要機能を一通り学びました。次のセクション（2-3-9〜2-3-16）では、スターターキットを使って各機能をより実務に近い形で実践していきます。
