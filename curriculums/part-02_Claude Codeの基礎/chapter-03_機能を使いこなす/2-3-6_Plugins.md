...
## 🏃 実践: Plugin をインストールし、/skill-creator で Plugin を作る

公式マーケットプレイスから Plugin をインストールし、さらに `/skill-creator` を使って自分の Plugin を作成する体験をしてみましょう。
...
### Step 4: /skill-creator で Plugin を作成する

Claude Code には `/skill-creator` という Skill があります。これは新しい Skill や Plugin の雛形を対話的に作成してくれるツールです。

```
> /skill-creator cc-practice 用のコードレビュー Plugin を作成して。JavaScript ファイルの命名規則とセキュリティの観点でレビューする Skill を含めて
```

`/skill-creator` が対話的に質問しながら、Plugin のディレクトリ構造（`plugin.json`、`skills/` 配下の SKILL.md）を自動生成します。

> 📝 あなたの環境では異なる内容が生成されますが、Plugin のディレクトリ構造（`.claude-plugin/plugin.json` と `skills/` 配下の SKILL.md）が作成されていれば成功です。
...
> 📝 ディレクトリ名は `/skill-creator` が生成したものに合わせてください。

Plugin が読み込まれたら、`/` と入力して Tab を押し、新しい Skill が一覧に表示されることを確認しましょう。
...
- [ ] **安全性**: 該当なし（ローカルでの Plugin 作成・テストのみ）
