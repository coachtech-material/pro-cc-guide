...
```mermaid
flowchart TD
    Input["あなたの指示:<br>「このファイルを修正して」"]
    Input --> Pre

    subgraph Lifecycle["Claude Code のライフサイクル"]
        Pre["① PreToolUse"]
        Tool["② ツール実行<br>（ファイル編集）"]
        Post["③ PostToolUse"]
        Stop["④ Stop"]
    ...
    Pre -.- H1[/"Hook: 保護ファイルの編集を阻止"/]
    Post -.- H2[/"Hook: Prettier で自動フォーマット"/]
    Stop -.- H3[/"Hook: デスクトップ通知を送信"/]
```
...
> 📝 `/hooks` コマンドを実行すると、設定済みの Hook の表示・追加・削除ができるインタラクティブなマネージャーが開きます。SessionStart、SessionEnd、Notification など、ここに挙げた以外にも多くのイベントがあります。Hook の種類も `command`（シェルコマンド）以外に、`http`（HTTP リクエスト）、`prompt`（LLM による判定）、`agent`（Sub-agent による検証）がありますが、この教材では最も基本的な `command` タイプに絞って解説します。
...
| `hooks` | 実行する Hook の配列。1つの `matcher` に対して複数の Hook を設定できる |
| `type` | Hook の種類。`command`（シェルコマンド）が最も一般的 |
| `command` | 実行するコマンド。標準入力で受け取った JSON からイベント情報を取得する |

`matcher` が重要です。`"Edit|Write"` と指定することで、ファイルの編集（Edit）または作成（Write）が行われたときだけ Hook が発火します。ファイルの読み取り（Read）や検索（Grep）では発火しません。これにより、不要な処理の実行を防げます。設定は二層構造になっており、外側の配列要素で `matcher`（発火条件）を定義し、その中の `hooks` 配列で実行する処理を定義します。
