...
> 📝 上記は macOS 用のコマンドです。Linux では `notify-send`、Windows では PowerShell の通知機能を使います。
...
Claude Code の作業完了時にデスクトップ通知を受け取る Hook を設定してみましょう。

### Step 1: settings.json を作成する

cc-practice プロジェクトで Claude Code を起動し、以下のように指示します。

```
> .claude/settings.local.json に以下の内容を書いて:
> Claude Code の作業が完了したらデスクトップ通知を送る Stop Hook を設定したい。
> macOS の osascript を使って「Claude Code の作業が完了しました」という通知を出すようにして
```
...
### Step 3: 設定済みの Hook を確認する

現在設定されている Hook の一覧を確認します。

```
> /hooks
```

先ほど設定した Stop Hook が表示されます。
