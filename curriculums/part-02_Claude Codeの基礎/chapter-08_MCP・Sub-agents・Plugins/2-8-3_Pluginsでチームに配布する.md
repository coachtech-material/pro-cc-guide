...
- [ ] **正しさ**: `/help` コマンドで `/tech-article-plugin:add-model` が表示されているか
- [ ] **正しさ**: 名前空間付きのコマンド（`/tech-article-plugin:add-model`）で Skill が正しく展開され、Claude Code が指示を受け取れるか
- [ ] **正しさ**: Plugin の Hooks（PostToolUse）が `/hooks` コマンドで確認でき、ファイル編集時に自動実行されるか
- [ ] **品質**: Plugin のディレクトリ構造が規約に従っているか（`.claude-plugin/plugin.json` にマニフェスト、`commands/` に Skill を配置）
- [ ] **品質**: `plugin.json` の `name`、`description`、`version` が適切に設定されているか
- [ ] **品質**: Standalone 設定（`.claude/`）から Plugin への移行時に、設定の内容が正しく引き継がれているか
- [ ] **安全性**: この Section では該当なし

> この Section では特に「正しさ」に注目してください。Plugin はディレクトリ構造のルール（`.claude-plugin/` にはマニフェストのみ、Skill は `commands/` 直下に配置）が厳密です。構造が間違っていると Skill が認識されないため、`/help` での確認を必ず行いましょう。
