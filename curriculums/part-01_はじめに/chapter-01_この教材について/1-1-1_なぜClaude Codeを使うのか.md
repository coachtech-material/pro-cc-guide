...
```mermaid
flowchart TD
    Goal["🎯 ゴール（抽象）<br>「注文機能を作りたい」<br>あなたが考える"]
    Goal -->|分解| Cart[カート管理]
    Goal -->|分解| Order[注文処理]
    Goal -->|分解| Stock[在庫管理]
    ...
    Cart -->|さらに分解| CartImpl["CartItem モデル<br>Cart Controller"]
    Order -->|さらに分解| OrderImpl["Order / OrderItem モデル<br>Order Controller"]
    Stock -->|さらに分解| StockImpl["stock カラム<br>在庫減算ロジック / Policy"]
