...
COACHTECH の選抜試験を突破し、フリーランスエージェントに所属しているあなたは、これから企業に紹介され、Laravel を使った実務開発に携わっていくことになります。
...
「AI に頼ると自分で考えなくなる」。AI コーディングツールに対して、そんな批判を耳にすることがあります。しかし実態はむしろ逆です。AI が定型的なコーディング作業を引き受けるようになった結果、人間には「何を作るべきか」「どう分解すべきか」「この出力は正しいか」という、より本質的な思考が求められるようになっています。
...
```
         ...
         │                      │                      │
   ┌─────┴─────┐          ┌─────┴─────┐          ┌─────┴─────┐
   │ カート管理 │          │  注文処理  │          │  在庫管理  │  あなたが構造化する
   └─────┬─────┘          └─────┬─────┘          └─────┬─────┘
         │                      │                      │ さらに分解
         ▼                      ▼                      ▼
   ┌───────────┐          ┌───────────┐          ┌───────────┐
   │ CartItem  │          │ Order     │          │ stock     │
   │ モデル     │          │ OrderItem │          │ カラム     │  AI が実装する
   │ Cart      │          │ モデル     │          │ 在庫減算   │
   │ Controller│          │ Order     │          │ ロジック   │
   │           │          │ Controller│          │ Policy    │
   └─────┬─────┘          └─────┬─────┘          └─────┬─────┘
         │                      │                      │
         └──────────────────────┼──────────────────────┘
