...
プログラミングスクール COACHTECH で学ぶジュニアエンジニア。Laravel を使った開発に取り組んでおり、基礎力はあるが実務経験は浅い。
...
AI で一人の生産性が上がり企業の採用が慎重になる中、ジュニアエンジニアがミドル・シニア相当のバリューを出せるよう Claude Code を使いこなせるようになることを目指す。Claude Code との協働を通じて**驚き**と**面白さ**を感じてもらいながら、実務で成果を出せるエンジニアに育てる。

教材の根底にある考え方: AI は抽象的な指示を具体のコードに高速で変換する。しかしその指示を MECE に構造化するには、具体を深く理解していなければならない。この **具体と抽象の行き来** を高速・高精度に回す力こそが AI 時代のエンジニアに求められるものである。
...
| Part 2: Claude Code の基礎（3 Chapter） | 使いこなす力 + 見極める力（型の導入） | 主要機能を習得（セットアップ → 基本理解 → 機能実践） | Ch1: 自分のプロジェクト作成 / Ch2: 座学 / Ch3: cc-practice で実践 |
| Part 3: Claude Code の実践（6 Chapter） | 見極める力 + 使いこなす力（総合演習） | 提供プロジェクト（CourseHub）で実務タスクを遂行（準備 → 理解 → バグ修正 → 機能開発 → 改善 → チーム共有） | CourseHub で実践 |
| Part 4: 継続的な学習（1 Chapter） | 学び続ける力 | 学び続けるサイクル・教材の総まとめ・発展機能の紹介 | なし |
...
│   └── coursehub-spec.md          # Part 3 提供プロジェクト仕様書
├── .claude/
│   ├── rules/
