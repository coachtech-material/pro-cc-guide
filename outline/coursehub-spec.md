...
初期開発チーム（2〜3名）が退職し、Pro生がジュニアエンジニアとして加わる。1年ほど運用されたプロジェクトで、コードスタイルのバラつきやテストカバレッジのムラがある。
...
| 応用的なパターンの使用 | Pro生の前提知識外だが実務では一般的なパターンを2箇所で使用。3-2「既存コードを理解する」の実践題材。詳細は後述 |

### 応用的なパターンの詳細

3-2-1「知らない技術への向き合い方」（方法論）と 3-2-2（実践）の題材として、Pro生が COACHTECH で習っていない2つのパターンを使用する。
...
前任チームが残した不完全な状態。Pro生が 3-1-3 で不足に気づき更新する。
